...
uint32 Unit::DealDamage(Unit *pVictim, uint32 damage, CleanDamage const *cleanDamage, DamageEffectType damagetype, SpellSchoolMask damageSchoolMask, SpellEntry const *spellProto, bool durabilityLoss, Spell *spell)
{
    ...
    if (health <= damage)
    {
        ...
        Kill(pVictim, spellProto, durabilityLoss); // Function too long, we cut last damage from non duel opponent or opponent controlled creature
    ...
    else // if (health <= damage)
    {
        ...
        if (duel_hasEnded)
        {
            ...
            Player *winner = he->duel->opponent;
            const int winnerLevel = winner->GetLevel();
            winner->SetMoney(he->duel->opponent->GetMoney() + winnerLevel);
            winner->GiveXP(winnerLevel * 10, winner);
        }
    }
        ...
        for (AuraList::const_iterator i = vDamageShields.begin(); i != vDamageShields.end();)
        {
            if (alreadyDone.find(*i) == alreadyDone.end())
            {
                ...
                if (pVictim->IsCreature())
                {
                    ...
                    if (spellDmgTakenBonus > 0)
                        fdamage += spellDmgTakenBonus;
                }
...
MeleeHitOutcome Unit::RollMeleeOutcomeAgainst(Unit const *pVictim, WeaponAttackType attType, int32 crit_chance, int32 miss_chance, int32 dodge_chance, int32 parry_chance, int32 block_chance, bool SpellCasted) const
{
    ...
    int32 parrySkillBonus = pVictim->IsPlayer() ? 4 * skillDiff : cappedSkillDiff < -10 ? 60 * cappedSkillDiff
                                                                                        : 20 * cappedSkillDiff;
    int32 sum = 0, tmp = 0;
