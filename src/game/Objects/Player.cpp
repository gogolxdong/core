...
SpellModifier::SpellModifier(SpellModOp _op, SpellModType _type, int32 _value, SpellEntry const *spellEntry, SpellEffectIndex eff, int16 _charges /*= 0*/) : op(_op), type(_type), charges(_charges), value(_value), spellId(spellEntry->Id), ownerAura(nullptr)
{
    mask = sSpellMgr.GetSpellAffectMask(spellEntry->Id, eff);
}

SpellModifier::SpellModifier(SpellModOp _op, SpellModType _type, int32 _value, Aura *aura, int16 _charges /*= 0*/) : op(_op), type(_type), charges(_charges), value(_value), spellId(aura->GetId()), ownerAura(aura)
{
...
bool Player::TeleportTo(uint32 mapid, float x, float y, float z, float orientation, uint32 options, std::function<void()> recover)
{
    ...
    BASIC_LOG("%d == %d  m_transport:%d", GetMapId(), mapid, m_transport);
...
bool Player::UpdateCraftSkill(uint32 spellid)
{
    ...
    for (SkillLineAbilityMap::const_iterator _spell_idx = bounds.first; _spell_idx != bounds.second; ++_spell_idx)
    {
        if (_spell_idx->second->skillId)
        {
            ...
            return UpdateSkillPro(_spell_idx->second->skillId, SkillGainChance(SkillValue, _spell_idx->second->max_value, (_spell_idx->second->max_value + _spell_idx->second->min_value) / 2, _spell_idx->second->min_value),
                                  craft_skill_gain);
        }
    }
    return false;
...
void Player::PrepareGossipMenu(WorldObject *pSource, uint32 menuId)
{
    ...
    if (canSeeQuests && ((pSource->IsGameObject() && pSource->ToGameObject()->GetGOInfo()->type != GAMEOBJECT_TYPE_QUESTGIVER) ||
...
bool Player::BuyItemFromVendor(ObjectGuid vendorGuid, uint32 item, uint8 count, uint8 bag, uint8 slot)
{
    ...
    auto playerRank = (sWorld.GetWowPatch() < WOW_PATCH_107) && sWorld.getConfig(CONFIG_BOOL_ACCURATE_PVP_PURCHASE_REQUIREMENTS) ? m_honorMgr.GetHighestRank().rank : m_honorMgr.GetRank().rank;
...
void Player::RewardHonorOnDeath()
{
    ...
    for (const auto &itr : m_damageTakenHistory)
    {
        ...
        {
            bool distance = attacker->IsAtGroupRewardDistance(this);
            bool alive = attacker->IsAlive();
            // sLog.outError("distance: %d\nalive: %d\n",distance, alive);
            if (Group *g = attacker->GetGroup()){
                g->GetLeaderGuid();
                damagePerGroup[g] += itr.second;
            }
            else if (distance && alive)
            {
